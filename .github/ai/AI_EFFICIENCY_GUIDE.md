...
- **One phase / one task per message.** Don't bundle "review X, do Y, plan Z,
  fix the build, verify". Batch a phase together, let the agent complete + gate
  it, *then* hand off the next.
...
- Large multiphase rewrites in a single shot. On very long diffs the agent's
  context drifts → it re-reads stale strings and misedits. Fatigue shows up as
...
- wasm-only behavioral reasoning. This environment has no wasm runtime and no
  ...
  that only runs in the browser should be native-verifiable first, then
  wasm-checked.
