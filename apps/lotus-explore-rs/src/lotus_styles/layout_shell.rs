...
@media (max-width: 768px) {
  .filters-toggle { display:flex; flex-wrap:wrap; align-items:center; justify-content:center; background:var(--btn-primary-bg); color:var(--text); border:0; border-radius:var(--radius-sm); padding:11px 16px; font-size:var(--fs-ui); font-weight:700; cursor:pointer; box-shadow:var(--shadow-xs); transition:background .15s, box-shadow .15s, transform .12s ease; text-align:center; line-height:1.2; white-space:normal; width:100%; min-width:0; box-sizing:border-box; }
