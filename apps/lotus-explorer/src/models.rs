...
impl CompoundEntry {
    pub fn compound_url(&self)  -> String { format!("https://www.wikidata.org/entity/{}", self.compound_qid) }
    pub fn taxon_url(&self)     -> String { format!("https://www.wikidata.org/entity/{}", self.taxon_qid) }
    pub fn reference_url(&self) -> String { format!("https://www.wikidata.org/entity/{}", self.reference_qid) }
    pub fn scholia_url(&self)   -> String { format!("https://scholia.toolforge.org/chemical/{}", self.compound_qid) }
    pub fn doi_url(&self)       -> Option<String> {
        self.ref_doi.as_ref().map(|d| format!("https://doi.org/{d}"))
    }
...
impl SearchCriteria {
    pub fn has_mass_filter(&self) -> bool { self.mass_min > 0.0 || self.mass_max < 2000.0 }
...
pub enum SmilesSearchType { Substructure, Similarity }
...
impl DatasetStats {
    pub fn from_entries(entries: &[CompoundEntry]) -> Self {
        ...
        Self {
            n_compounds:  entries.iter().map(|e| e.compound_qid.as_str()).collect::<HashSet<_>>().len(),
            n_taxa:       entries.iter().map(|e| e.taxon_qid.as_str()).collect::<HashSet<_>>().len(),
            n_references: entries.iter().map(|e| e.reference_qid.as_str()).collect::<HashSet<_>>().len(),
...
#[derive(Debug, Clone, Copy, PartialEq)]
pub enum SortColumn { Name, Mass, Formula, TaxonName, PubYear, RefTitle }

#[derive(Debug, Clone, Copy, PartialEq)]
pub enum SortDir { Asc, Desc }

#[derive(Debug, Clone)]
pub struct SortState { pub col: SortColumn, pub dir: SortDir }

impl Default for SortState {
    fn default() -> Self { Self { col: SortColumn::Name, dir: SortDir::Asc } }
}
