...
This document describes the three-level hierarchical lipid classification system used in lipid-selecto-rs.
...
Rather than hardcoding full patterns, we define reusable SMARTS cores that represent actual chemical structures:
...
**Correct approach:**
```
backbone = identify_backbone()  // glycerol vs. sphingoid
acyl_count = count_acyl_esters()
...
if backbone == glycerol:
    if acyl_count == 2:
        ...
        → LPC
elif backbone == sphingoid:
    → SM
```

### Ether vs Acyl vs Plasmalogen
These must be distinguished in architecture, not just presence of functional groups.
